function criarPergunta() {
    ...
    tipoInput.name = "tipo_pergunta[]"; // pode ser "dissertativa", "objetiva", "classificacao"
    ...
    try {
        ...
        perguntasItems.forEach((item) => {
            ...
            if (pergunta.tipo === "objetiva") {
                pergunta.opcoes = Array.from(item.querySelectorAll('.opcao-item input[type="text"]')).map(
                    (input) => input.value,
                );
            }
...
document.querySelectorAll('.btn-exportar').forEach(button => {
  button.addEventListener('click', function () {
    const id = this.getAttribute('data-id');
    if (id) {
      // Redireciona automaticamente para o exportador passando o ID
      window.location.href = `exportar_formulario.php?id=${id}`;
    }
  });
});